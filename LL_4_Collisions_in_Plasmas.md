...
!!! info
    英語版Lifshitz Pitaevskii Physical Kineticsの議論を大雑把に邦訳しつつ、私の理解度に合わせて行間を詰めたノートです。
    
    Landau減衰でプラズマ波（静電場と電子密度揺動）がダンプされた後にも分布関数レベルでは揺動が残ります。このような揺動がクーロン散乱で減衰する時間スケールについて論じられています。減衰の特徴時間が単に衝突周波数にならない点が少し意外でした。
...
ここで、$\delta f$は平衡状態の分布関数$f_0$からの摂動を表す。そして、$a(v,t)$はゆっくりと変化する速度の関数である。ゆっくりとはすなわち$\sim \bar{v} \gg 1/kt$程度で、あらわに変化する程度である。
...
$$
\begin{align*}
I_{ij}(v) = &\int d^3 v' f_0 (v') \frac{(v-v')^2 \delta_{ij}- (v_{i}-v'_{i})  (v_{j}-v'_{j}) }{|v-v'|^3}
\end{align*}
$$
...
例えば、$v \neq0$のとき、$O_{ij}v_j=w_i = v \delta_{i3}, \,(w_1=0, w_2 = 0, w_3 =v)$となるような直交変換$O_{ik}O_{jk}=O_{ki}O_{kj} = \delta_{ij}$を与えて、作用すると、
...
O_{ki}O_{lj}I_{ij}(v) 
= 
&\int d^3 v' f_0 (v') \frac{(v-v')^2 \delta_{kl}- (v\delta_{k3}- O_{ki}v'_{i})  (v\delta_{l3}- O_{lj}v'_{j}) }{|v-v'|^3}\\
\end{align*}
$$

$u_i = O_{ij} v'_j$と積分変数を取り替えて、

$$
\begin{align*}
 I'_{kl} \equiv O_{ki}O_{lj}I_{ij}(v) = &\int d^3 u f_0 (u) \frac{(w-u)^2 \delta_{kl}- (v\delta_{k3}- u_k)  (v\delta_{j3}- u_l) }{|w-u|^3} \\
\end{align*}
$$

$$
    I_{ij} = O_{ki}O_{lj} I'_{kl}
$$

ここで、$d^3u = d^3v'$。回転対称の仮定から$f_0(v'_i) = f_0(O_{ik} v_k) =f_0(u_k)$を使った。

$I'$は座標を取り替えて、ベクトル$v$をわかりやすい位置に持ってきた時のテンソルに相当している。
成分を具体的に見れば、
...
$$
\begin{align*}
    I'_{12} = &\int d^3 u f_0(u)\frac{- u_1u_2}{|w-u|^3}=0\\
\end{align*}
$$
...
$$
\begin{align*}
I'_{13} = &\int d^3 u f_0(u)\frac{u_1(v-u_2)}{|w-u|^3}=0\\
\end{align*}
$$

テンソルの非対角成分${k\neq l}$をみると、奇関数なので、0になることがわかる。
すなわちこの座標系でテンソルが対角化されている。続いて対角成分を確認すると、被積分関数が正なので、積分で相殺して0になるようなことも起こらないことが確認できる。
...
\begin{align*}
I'_{11} = I'_{22} 
&=\int d^3 uf_0(u)\frac{v^2-2vu_3+u_3^2+u_2^2}{|w-u|^3}\\
&=\int d^3 uf_0(u)\frac{v^2-2vu_3+u_3^2+u_1^2}{|w-u|^3}\\
&=\int d^3 uf_0(u)\frac{(v-u_3)^2 + u^2}{2|w-u|^3} \quad >0
\end{align*}
$$
...
\begin{align*}
I'_{3,3}(v) 
&= \int d^3 uf_0(u)\frac{(w-u)^2-(v-u_3)^2}{|w-u|^3}\\
&= \int d^3 uf_0(u)\frac{u_1^2+u_2^2}{|w-u|^3} \quad > 0\\
\end{align*}
$$


対角化されたテンソルを$w$を使って書くと座標不変な形式でかけることがわかる。
...
$$  
\begin{align*}
    I'_{kl} &=  I_{\perp} (\delta_{kl}- \delta_{k3} \delta_{l3}) + I_{/\!/} \delta_{k3} \delta_{l3}\\
            &= I_{\perp} \left( \delta_{kl}- \frac{w_k w_l}{v^2} \right ) + I_{/\!/}  \frac{w_k w_l}{v^2} \\
    I_{ij}  &= O_{ki}O_{lj} I'_{kl} \\
        &= I_{\perp} \left( \delta_{ij}- \frac{v_i v_j}{v^2} \right ) + I_{/\!/}  \frac{v_i v_j}{v^2} \\
\end{align*}
$$
...
$$
\begin{align*}
    I_{/\!/} = I'_{3,3} (v) &= \sqrt {\frac{1}{2\pi v_T^2}}^3 \int d^3 u \, e^{-(u^2 /2v_T^2)}\frac{u_1^2+u_2^2}{|w-u|^3} \\
...
$v/v_T \ll1$の時、非積分関数はほとんどuの関数かつ分子分母は同次なので、オーダー１つまり、$I_{/\!/}\sim 1/v_T$。
