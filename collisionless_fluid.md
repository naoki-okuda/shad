...
一本目が質量保存則、二本目が運動量保存則、三本目がエネルギー保存則に応じている。四・五本目が構成方程式だ。四本目は内部エネルギーが流体素片の重心速度によるエネルギーと重心周りの相対運動エネルギーに分解できるということ、五本目は相対運動エネルギーが熱的な内部エネルギーとして圧力と関係付けられることが仮定されている。

!!! note
    流体系の時間発展のベースに数密度・運動量・エネルギー保存則があるという物理的観点は重要で、「流体」という取り扱いの適用範囲の広さの一つの正体だ。媒質の詳細な性質を無視して、これらの保存則を満たせばそれで良いという開き直りを正当化する。物性の詳細な情報は、構成方程式、応力テンソル、輸送係数、外力の構成の工夫に押し込められる。
