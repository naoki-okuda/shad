...
まずは、圧縮性理想流体を記述するEuler方程式を概観する。流体方程式の基本的な構成は
...
ここで、添字は$(i=1,2,3)$で空間成分を表し、記号の重複に対して和をとることとする($A^i B^i = A^1B^1 + A^2 B^2 + A^3 B^3$)。
$u^i$は流体素片の平均速度、$\rho$は質量密度、$p$は圧力、$F$は流体素片が受ける平均的な外力、$\epsilon$は単位質量あたりの内部エネルギー、$\gamma$は比熱比。

一本目が質量保存則、二本目が運動量保存則、三本目がエネルギー保存則に対応している。四・五本目が構成方程式を表している。四本目は内部エネルギーが流体素片の重心速度によるエネルギーと重心周りの相対運動エネルギーに分解できるということ、五本目は相対運動エネルギーが熱的な内部エネルギーとして圧力と関係付けられることが仮定されている。

!!! note
    流体系の時間発展のベースに数密度・運動量・エネルギー保存則があるという物理的観点は重要で、「流体」という取り扱いの適用範囲の広さの一つの正体だといえる。媒質の詳細な性質を無視して保存則さえ満たせば良いという開き直りによって多種多様な物質に対する流体方程式の適用が正当化される。ただし、物性の詳細な情報は、構成方程式、応力テンソル、輸送係数、外力の構成の工夫に押し込められる。


Euler方程式系及びより一般の流体方程式の形式を導出・納得する方法は様々だろう。
一つの重要な方法は、速度分布関数の時間発展を記述するクラス（いわゆるphysical kinetics・ボルツマン方程式）を出発点にする方法だ。

このやり方の恩恵は、

- 保存形式が見えやすいこと
- 流体方程式を覚えずとも復元できるようになること
- 輸送係数の出所がわかりやすくなること
- 質点の力学と流体力学のギャップが埋まること
...
!!! info
    無衝突ボルツマン方程式を出発点にする。記事の前半では速度分布関数の整数次モーメントを取ることで保存形式のセットを得る。後半では、熱化の視点から構成方程式について説明する。　　
    無衝突の仮定から原理的には熱化が入らないので、厳格な導出にはならないことには注意する。
...
$$
    0 = \nabla_v \cdot F = \frac{\partial F^i}{\partial v^i} 
$$

ボルツマン方程式から衝突項を落としたものに相当しているので、無衝突ボルツマン方程式(collisionless Boltzmann equation)と呼ばれる。

!!! note 
    一本目の式は$f(x,v,t)$の発展方程式を与えている。移流方程式の形式から、位相空間上の流れに沿って$f$が保存すると言っている。すなわちラグランジュ微分

    $$
        \frac{D }{Dt} = \frac{\partial }{\partial t} + v \cdot \nabla_x + F \cdot \nabla_v
    $$

    に対して、$f$が一定だと読める。二本目の拘束条件から、保存形式に書き直すことができる。すなわち、
    
    $$
    0=   
    \frac{\partial f}{\partial t} 
    + \nabla_{xv} \cdot (f\Gamma) , \quad
    \Gamma = 
    \begin{pmatrix} 
    v\\
    F
    ...
    $f$は位相空間上の粒子数密度・あるいは確率密度のようなものなので、こちらの形式の方が顕に保存則を反映した書き方となっている。
    翻って、条件$\nabla_v \cdot F = 0$は、六次元位相空間上の流れに対する「非圧縮条件」だと解釈できる。流体力学のおいて、流れ場が非圧縮であれば、$\nabla_x \cdot u = 0$、質量保存則から流れに沿った密度変化が0になる($Df/dt = 0$)であることと構造的に同じだ。今、流れ場が非圧縮($\nabla \cdot \Gamma$ = 0)なので、数密度$f$のラグランジュ微分が0になる($Df/Dt = (\partial_t + \Gamma \cdot \nabla_{xv})f = 0$)。


!!! note 
    流れ場の非圧縮性$\nabla_v \cdot F = 0$は、多粒子に対する平均場的な力$F(x,v,t)$が、時間発展で六次元位相空間体積($d^3 x d^3 v$)を不変に保つことを要請している。いわゆる、「Liouvilleの定理」が$6N$次元から縮減された6次元上の分布関数でも成立することを要請している。
    Liouville性は力学のハミルトン構造に由来している。
    正準変数$(x,p)$のハミルトンフローを考えると、
    $$
    (\dot{x}, \dot{p}) = \left(\frac{\partial H}{\partial p}, -\frac{\partial H}{\partial x}\right)
    $$
    で、位相空間での発散$\nabla_{xp} = (\nabla_x, \nabla_p)$を取ると、
    $$
    \nabla_{xp} \cdot \left(\frac{\partial H}{\partial p}, -\frac{\partial H}{\partial x}\right) = 0
    $$
    となる。
    例えば、ローレンツ力$F^i = q(E^i + \epsilon^{ijk}v^j B^k)$はこれを満たす(→ Vlasov 方程式)。  

!!! note
    流れ場の非圧縮条件を満たす力$\nabla_v \cdot F = 0$を考える場合、相空間の発展方程式は移流形式$\frac{df}{dt}=0$へと簡略化される。つまり力の因子$F^i$が時間偏微分の外に出された移流形式の方が「無衝突ボルツマン方程式」及び、「Vlasov方程式」と呼ばれることが多い。
    もし摩擦$F^i = -3 \nu m v^i$のような散逸的な力を「外力」としてこの形式に強引に組み込むと、$\nabla_v \cdot F = -3 \nu m$となり、粒子の軌道に沿って分布関数の値 f が指数関数的に増大（速度空間での濃縮）することになる。数学的には保存形式のまま扱うことも可能だが、物理的にはこのような散逸効果は「外力項」として左辺に置くよりも、背景粒子との相互作用（衝突項）として右辺の Fokker-Planck演算子などで記述されることが多い。
...
になる。温度は$T = \frac{p}{n}$として後から定義される。
