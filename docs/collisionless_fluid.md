...
希薄は流体に対する、最もシンプルで汎用性の高い近似は、分布関数を局所的にマクスウェル分布とすることである。平均速度$u$のマクスウェル分布は(shifted maxwellian)、

$$
f \approx f_M \propto e^{ [-m(v-u)^2/2T]}
$$

shifted maxwellianで打ち切れば、圧力テンソルは等方になり理想気体の状態方程式も入る。また、平均流周りの奇数次モーメントは0になる。
